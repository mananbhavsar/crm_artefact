...
class Supplier_Model extends CI_Model {
	...
	function get_supplier_id( $id ) {
    	...
    	function get_supp_contact_records($id){
	    	$this->db->select( '*' );
	    
		return $this->db->get_where( 'supplier_contact', array( 'supplierid' => $id ) )->result_array();
	...
	function update_file($image_name,$id,$type){
        if($type == 'trade_documents'){
        $sql = $this->db->query("UPDATE  supplier SET `trade_licence_documents` = replace(replace(trade_licence_documents, '$image_name', ''), '@@' , '@') WHERE supplier_id = '$id'");
        }
        if($type == 'tax_documents'){
              $sql = $this->db->query("UPDATE  supplier SET `tax_registration_document` = replace(replace(tax_registration_document, '$image_name', ''), '@@' , '@') WHERE supplier_id = '$id'");
        }
         if($type == 'Labour'){
              $sql = $this->db->query("UPDATE  supplier SET `owner_documents` = replace(replace(owner_documents, '$image_name', ''), '@@' , '@') WHERE supplier_id = '$id'");
        }
	...
	function add_supplier_contact( $params ) {
		$this->db->insert( 'supplier_contact', $params );
	...
	{
		$this->db->select('*');
		$this->db->join( 'supplier_material', 'supplier_material.supplier_id = supplier.supplier_id', 'left' );
		
		$this->db->group_by('supplier_material.supplier_id'); 
		return $this->db->get_where( 'supplier', array( '' ) )->result_array();
	}
	function supplier_material_relation()
	{
		$this->db->select('*');

		return $this->db->get_where( 'supplier_material', array( '' ) )->result_array();
	}
	...
	function update_supplier( $id, $params ) {
		...
		function delete_supplier_contacts($supplier_id){
		    
	   	$response = $this->db->delete('supplier_contact', array( 'supplierid' => $supplier_id));
	    return $response;
	}
	
	function delete_supplier($supplier_id){
	    
	   $response = $this->db->delete('supplier', array( 'supplier_id' => $supplier_id));
	    return $response;
	}
