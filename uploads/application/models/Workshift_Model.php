<?php
class Workshift_Model extends CI_Model {
	...
	function get_workshift_record($id){
	    	$this->db->select( '*' );
		return $this->db->get_where( 'workshift', array( 'shift_id' => $id ) )->row_array();
		
	}
	function get_department_name($depts){
		$sql = "SELECT name FROM departments WHERE FIND_IN_SET(id,'$depts')";
		$res = $this->db->query($sql);
		$result = $res->result_array(); 
		return $result;
	}
	function get_emp_name($emps){
		$sql = "SELECT staffname FROM staff WHERE FIND_IN_SET(id,'$emps')";
		$res = $this->db->query($sql);
		$result = $res->result_array(); 
		return $result;
	}
	function getshiftrecord($emp){
	    
	    $sql = "SELECT * FROM workshift WHERE FIND_IN_SET('$emp',employee_id)";
//echo $sql; die;
	    $res = $this->db->query($sql);
	    $result = $res->result_array();
	    return $result;
	...
	function get_item_code($item_code){
		...
		function get_workshift_break_ecords($id){
	    	$this->db->select( '*' );
		return $this->db->get_where( 'workshift_break_timings', array( 'shift_id' => $id ) )->result_array();
...
function get_all_suppliers_material($id) {
	...
	function delete_shift($shift_id) {
		return $this->db->delete('workshift', array( 'shift_id' => $shift_id));
	}
	
	function delete_shiftbreak_records($shift_id){
	   	return $this->db->delete('workshift_break_timings', array( 'shift_id' => $shift_id));
    ...
    function update_shift($id, $params) {
		$response = $this->db->where('shift_id', $id)->update('workshift', $params);
	...
	function get_all_shifts($type="") {
		
		$this->db->order_by("workshift.shift_id ","desc");
		$this->db->select( '*' );
		//$this->db->join( 'material_unit_type', 'materials.unittype = //material_unit_type.unit_type_id' );
		return $this->db->get_where( 'workshift', array( '' ) )->result_array();
	}
	...
	function add_shift( $params ) {
		$this->db->insert( 'workshift', $params );
		$shift_id = $this->db->insert_id();
		
		return $shift_id;
	}
	function add_workshift_items( $params ) {
		$this->db->insert( 'workshift_break_timings', $params );
...
if($row){
 ...
 return 0;   
    ...
    function update_file($image_name,$id){
        ...
        $sql = $this->db->query("UPDATE materials SET `documents` = replace(replace(documents, '$image_name', ''), '@@' , '@') WHERE material_id = '$id'");
