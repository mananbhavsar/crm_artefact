...
function get_salestargetdata($staff_id) {
	$ci =& get_instance();
		$id = $ci->session->userdata( 'usr_id' );
		$ci->db->select( '*' );
		$ci->db->join( 'salestarget', 'staff.id = salestarget.user_id', 'inner' );
		$salestarget = $ci->db->get_where( 'salestarget', array( 'salestarget.user_id' => $staff_id ) )->row_array();
