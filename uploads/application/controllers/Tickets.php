...
class Tickets extends CIUIS_Controller {
    ...
    function assignto_staff($id){
        
        if ( $this->Privileges_Model->check_privilege( 'tickets', 'edit' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
			    $ticket_result = $this->Tickets_Model->get_tickets($id);
			    $emps_nms = $this->Tickets_Model->get_employee_name($ticket_result['employee_id'],$id);
			   //print_r($emps_nms); die;
			    $data['name'] = $emps_nms['empname'];
			    $data['success'] = true;
	...
	function assign_staff( $id ) {
		if ( $this->Privileges_Model->check_privilege( 'tickets', 'edit' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				$params = array(
					'ticket_id' => $id,
					'employee_id' => $ticket_result['employee_id'].','.$this->input->post( 'staff' ),
				);
				$response = $this->db->where( 'id', $id )->update( 'tickets', array( 'employee_id' => $ticket_result['employee_id'].','.$this->input->post( 'staff' ) ) );
				
				$Notifyparams=array();
				$Notifyparams['staffList']=$employeeids;
				$Notifyparams['msg']=( '' .$ticket_result['ticket_number'].' '.lang('assigned').' By '. $this->session->staffname .'' );
				$Notifyparams['link']= base_url('tickets/ticket/' . $id . '');
				$Notifyparams['staff_id']= $this->input->post( 'staff' );
				...
				$data['name'] = $emps_nms['empname'].','.$user[ 'staffname' ];
	...
	function markas() {
		if ( $this->Privileges_Model->check_privilege( 'tickets', 'edit' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				$tickets_id = $this->Tickets_Model->update_tickets($_POST[ 'ticket_id' ], $params );
				$data['success'] = true;
	...
	function get_ticket( $id ) {
		if ( $this->Privileges_Model->check_privilege( 'tickets', 'all' ) ) {
			$ticket = $this->Tickets_Model->get_ticket_by_privileges( $id );
			$emps_nms = $this->Tickets_Model->get_employee_name($ticket['employee_id'],$id);
			//print_r($emp_nms['empname']); die;
		} else if ($this->Privileges_Model->check_privilege( 'tickets', 'own') ) {
			$ticket = $this->Tickets_Model->get_ticket_by_privileges( $id, $this->session->usr_id );
				$emps_nms = $this->Tickets_Model->get_employee_name($ticket['employee_id'],$id);
				$ass_staff = $this->Tickets_Model->get_ticket_record($this->session->usr_id,$id);
		} else {
			$this->session->set_flashdata( 'ntf3',lang( 'you_dont_have_permission' ) );
			redirect(base_url('tickets'));
		}
		if($ticket) {
			...
			$data_ticketdetails = array(
				...
				'replies' => $replies,
				'role_id' => $ticket['srole_id'],
				'ass_staff_id' => 	$ass_staff['staff_id'],
				'ticket_number' => get_number('tickets', $ticket[ 'id' ], 'ticket','ticket'),
			);
			//print_r($data_ticketdetails); die;
