...
class Orders extends CIUIS_Controller {
	...
	function order( $id ) {
		...
		$rel_type = $pro[ 'relation_type' ];
		/*if ( $this->Privileges_Model->check_privilege( 'orders', 'all' ) ) {
			$ordersdata = $this->Orders_Model->get_order_by_priviliges( $id, $rel_type );
		} else if ($this->Privileges_Model->check_privilege( 'orders', 'own') ) {
			$ordersdata  = $this->Orders_Model->get_order_by_priviliges( $id, $rel_type, $this->session->usr_id );
		} else {
			$this->session->set_flashdata( 'ntf3',lang( 'you_dont_have_permission' ) );
			redirect(base_url('orders'));
		}*/
		...
		$this->db->order_by( 'rejected_notes.id', 'desc' );
		$data['declined_notes'] = $this->db->get_where( 'rejected_notes', array( 'relation' => $id, 'relation_type' => 'orders'))->row_array();
		
		/*$data['rejected_notes'] = $this->db->get_where( 'rejected_notes', array( 'relation' => $id, 'relation_type' => 'order', 'status'=>'Rejected'))->row_array();*/
	...
	function get_orders() {
		...
		foreach ( $orders as $order ) {
			...
			switch ( $order[ 'status_id' ] ) {
				...
				case '10':
					$status = lang( 'sendforapproval' );
					$class = 'order-status-sendforapproval proposal-status-sentforapproval';
					break;
				case '11':
					$status = lang( 'rejected' );
