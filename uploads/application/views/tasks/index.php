          ...
          <table md-table  md-progress="promise" ng-cloak>
            <thead md-head md-order="task_list.order">
              <tr md-row>
				...
				<th ><span><?php echo lang('assignedto'); ?></span></th>
				<th ><span><?php echo lang('assignedby'); ?></span></th>
              </tr>
            </thead>
            <tbody md-body>
			  ...
			  <!--ng-click="goToLink('tasks/task/'+task.id)"-->
                <td md-cell>
                  <strong>
                    <a class="link" ng-href="<?php echo base_url('tasks/task/')?>{{task.id}}"> <strong ng-bind="task.name"></strong></a> <br>
                    <small ng-bind="task.description"></small>
                ...
                <td md-cell>
				<div ng-if="task.statusedit==1">
				
   <select name="statuscha"  onchange="changestatus(this.value)" class="form-control" id="" style="width: 132px;">

   <option value="{{task.id}}id1" id="1" ng-selected="task.status_id=='1'" ng-style="myOptionStyle();"><?php echo lang('open'); ?></option>
				<option value="{{task.id}}id2"  id="2" ng-selected="task.status_id=='2'" ><?php echo lang('inprogress'); ?></option>
				<option value="{{task.id}}id3"   id="3" ng-selected="task.status_id=='3'"><?php echo lang('waiting'); ?></option>
				<option value="{{task.id}}id4"  id="4" ng-selected="task.status_id=='4'"><?php echo lang('complete'); ?></option></select>
</div>
<div ng-if="task.statusedit==0">
     <span>
         <div ng-if="task.status_id==1">
                    <b ng-bind="task.status" style="color:red;"></b>
            </div>
            <div ng-if="task.status_id==2">
            <b ng-bind="task.status"></b>
            </div>
            <div ng-if="task.status_id==3">
            <b ng-bind="task.status"></b>
            </div>
            <div ng-if="task.status_id==4">
            <b ng-bind="task.status"></b>
            </div>
				 ...
				 <td  >
                  <div class="bottom-right text-right">
                    <ul class="more-avatar">
                      <li ng-repeat="assigned in task.assignedby" data-toggle="tooltip" data-placement="left" data-container="body" title="" data-original-title="{{assigned.staffname}}">
                        <md-tooltip md-direction="top">{{assigned.staffname}}</md-tooltip>
                        <div style=" background: lightgray url({{UPIMGURL}}{{assigned.staffavatar}}) no-repeat center / cover;"></div>
                      </li>
  ...
  <md-sidenav class="md-sidenav-right md-whiteframe-4dp" md-component-id="ContentFilter" ng-cloak style="width: 450px;">
    ...
    <md-content layout-padding="">
      <div ng-repeat="(prop, ignoredValue) in tasks[0]" ng-init="filter[prop]={}" ng-if="prop != 'id'  && prop != 'name' && prop != 'relationtype' && prop != 'duedate' && prop != 'startdate' && prop != 'status' && prop != 'done' && prop != 'status_id' && prop != 'task_number' && prop != 'members'  && prop != 'priority' && prop != 'assignedby' && prop != 'statusedit' && prop != 'description'">
...
function changestatus(str){
	...
	$.ajax({
        url: '<?php print base_url();?>tasks/update_status/'+str+'',
        type: 'GET',
