  ...
  <div class="main-content container-fluid col-xs-12 col-md-12 col-lg-3 md-pl-0">
    ...
    <div class="col-md-12 col-xs-12 md-pr-0 md-pl-0 md-pb-10" style="background: white">
      ...
      <div class="col-xs-12 task-sidebar-item" ng-cloak>
        <ul class="list-inline task-dates">
          <li class="col-md-6 col-xs-6">
            <h5><?php echo lang('assignedto') ?></h5>
            <span class="mdi mdi-assignment-account"></span> <strong ><?php print $staffname;?></strong> </li>
          <li class="col-md-6 col-xs-6">
            <h5><?php echo lang('assignedby') ?></h5>
			<?php print $assignedby['staffname'];?>
			<!--
  ...
  <md-sidenav class="md-sidenav-right md-whiteframe-4dp" md-component-id="Update" style="min-width: 450px;" ng-cloak>
    ...
    <md-content>
      <md-content layout-padding>
		...
		<?php //echo "<pre>";
		//print_r($task);?>
		<form action="<?php print base_url('tasks/update/'.$task['id'])?>" method="post" id="formid" enctype="multipart/form-data">
	 <div class="form-group">
	   <label><strong><?php echo lang('task').' ' .lang('name') ?><span class="text-danger">*</span></strong></label>
          <input required type="text"  class="form-control" id="title" placeholder="<?php echo lang('name'); ?>" name="name" value="<?php if(isset($task)){print $task['name'];}?>"/>
		  </div>
...
<?php
	foreach($projects as $eachProj){
		?>
		<option value="<?php print $eachProj['id'];?>" <?php if(isset($task) && $task['relation']==$eachProj['id']){print "selected='selected'";}?>><?php print $eachProj['name'];?></option>
		<?php 
	}?>
	</select>
	<?php 
...
<?php
	foreach($staffs as $eachstaffs){
		?>
		<option value="<?php print $eachstaffs['id'];?>" <?php if(in_array($eachstaffs['id'],$staffid)){print "selected='selected'";}?>><?php print $eachstaffs['staffname'];?></option>
		<?php 
	}?>
	</select>
	<?php 
		  ...
		  <select onchange="showdateselection(this.value)" name="priority" id="priority" class="text-uppercase selectpicker" data-width="100%">
			<option value="1" <?php if(isset($task) && $task['priority']==1){print "selected='selected'";}?>>Low</option>
			<option value="2" <?php if(isset($task) && $task['priority']==2){print "selected='selected'";}?>>Medium</option>
			<option value="3" <?php if(isset($task) && $task['priority']==3){print "selected='selected'";}?>>High</option>
			<option value="4" <?php if(isset($task) && $task['priority']==4){print "selected='selected'";}?>>Due Date</option>
		  </select>
		  <!--
		  ...
		  <div class="form-group" <?php if(isset($task) && $task['priority']==4){print 'style="display:block;"';}else{print 'style="display:none;"';}?>id="duedateselect">
<label ><strong><?php echo lang('task').' ' .lang('duedate') ?><span class="text-danger">*</span></strong></label>


  <input mdc-datetime-picker="" date="true" time="false" type="text" id="datetime1" placeholder="<?php echo lang('chooseadate') ?>" show-todays-date="true" minutes="false" min-date="date" show-icon="false" ng-model="task.duedate" class=" form-control" name="duedate" value="<?php print $task['duedate']; ?>" readonly>
		  ...
		  <button type="button" class="btn btn-report" onclick="checkvalidations();">Update</button>
