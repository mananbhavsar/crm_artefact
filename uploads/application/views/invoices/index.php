		...
		<div class="main-content container-fluid col-xs-12 col-md-12 col-lg-9">
			...
			<md-toolbar class="toolbar-white">
				<div class="md-toolbar-tools">
					<h2 flex md-truncate class="text-bold"><?php echo lang('invoices'); ?> <small>(<span ng-bind="invoices.length"></span>)</small><br><small flex md-truncate><?php echo lang('organizeyourinvoices'); ?></small></h2>
					<div class="ciuis-external-search-in-table">
						<input ng-model="invoice_search" class="search-table-external" id="search" name="search" type="text" placeholder="<?php echo lang('search_by').' '.lang('customer')?>"> 
					...
					<?php if (check_privilege('invoices', 'create')) { ?>
						<md-button ng-href="<?php echo base_url('invoices/create') ?>" class="md-icon-button" aria-label="New" ng-cloak>
			...
			<div ng-show="invoiceLoader" layout-align="center center" class="text-center" id="circular_loader">
				...
				<p style="font-size: 15px;margin-bottom: 5%;">
					<span>
						...
						<small><strong><?php echo lang('loading'). ' '. lang('invoices').'...' ?></strong></small>
					</span>
				</p>
			</div>
					...
					<table md-table  md-progress="promise">
						<thead md-head md-order="invoice_list.order">
							<tr md-row>
								...
								<th md-column md-order-by="paid"><span><?php echo lang('paid'); ?></span></th>
								<th md-column md-order-by="due"><span><?php echo lang('due'); ?></span></th>
								<th md-column md-order-by="status"><span><?php echo lang('status'); ?></span></th>
							</tr>
						</thead>
						<tbody md-body>
							<tr class="select_row" md-row ng-repeat="invoice in invoices | orderBy: invoice_list.order | limitTo: invoice_list.limit : (invoice_list.page -1) * invoice_list.limit | filter: invoice_search | filter: FilteredData" class="cursor" ng-click="goToLink('invoices/invoice/'+invoice.id)">
								<td md-cell>
									<strong>
										<a class="link" ng-href="<?php echo base_url('invoices/invoice/') ?>{{invoice.id}}"> <span ng-bind="invoice.longid"></span></a>
									</strong><br>
								...
								<td md-cell>
									<strong ng-bind-html="invoice.paymentamount | currencyFormat:cur_code:null:true:cur_lct"></strong>
								</td>
								<td md-cell>
									<strong ng-bind-html="invoice.dueamount | currencyFormat:cur_code:null:true:cur_lct"></strong>
								</td>
							</tr>
						</tbody>
					</table>
		...
		function ReportOverviewConstructor(options) {
			...
			function init() {
				...
				$('.invoice-percent').percentCircle({
					...
					barColor: '#22c39e',
					barWeight: 3,
					endPercent: 0.<?php echo $amountpercentpaid ?>,
					fps: 60
				});
				$('.invoice-percent-2').percentCircle({
					...
					barColor: '#ee7a6b',
					barWeight: 3,
					endPercent: 0.<?php echo $amountpercentunpaid ?>,
					fps: 60
				});
				$('.invoice-percent-4').percentCircle({
					width: 130,
					trackColor: '#ececec',
					barColor: '#808281',
					barWeight: 3,
					endPercent: 0.<?php echo $amountpercentpartialpaid ?>,
					fps: 60
				});
				...
				$('.invoice-percent-3').percentCircle({
					...
					barColor: '#808281',
					barWeight: 3,
					endPercent: 0.<?php echo $amountpercentduepaid ?>,
					fps: 60
				});
			}
			function getTemplateString()
			{
				...
				'<div class="percentage">%<?php echo $amountpercentpaid ?></div>',
				'</div>',
				'<div class="box-foot">',
				...
				'<div class="box-foot-right"><br><span class="box-foot-stats""><strong><?php echo $otf ?></strong> (%<?php echo $amountpercentpaid ?>)</span></div>',
				'</div>',
				'</div>',
				'<div class="ciuis-right-border-b1 ciuis-invoice-summaries-b1">',
				'<div class="box-header text-uppercase text-bold"><?php echo lang('unpaidinvoice'); ?></div>',
				'<div class="box-content invoice-percent-2">',
				'<div class="percentage">%<?php echo $amountpercentunpaid ?></div>',
				'</div>',
				'<div class="box-foot">',
				'<span class="arrow arrow-up"></span>',
				...
				'<span class="arrow arrow-down"></span>',
				'<div class="box-foot-right"><br><span class="box-foot-stats"><strong><?php echo $tef ?></strong> (%<?php echo $amountpercentunpaid ?>)</span></div>',
				'</div>',
				'</div>',
				'<div class="ciuis-right-border-b1 ciuis-invoice-summaries-b1">',
				'<div class="box-header text-uppercase text-bold"><?php echo lang('partial'); ?></div>',
				'<div class="box-content invoice-percent-4">',
				'<div class="percentage">%<?php echo $amountpercentpartialpaid ?></div>',
				'</div>',
				'<div class="box-foot">',
				'<span class="arrow arrow-up"></span>',
				'<div class="box-foot-left"><?php echo lang('invoiceamount'); ?><br><span class="box-foot-stats"><strong><?php echo amount_format($parf, true) ?></strong></span></div>'.replace(/{{OdenmeyenInvoicesAmount}}/, options.data.OdenmeyenInvoicesAmount),
				'<span class="arrow arrow-down"></span>',
				'<div class="box-foot-right"><br><span class="box-foot-stats"><strong><?php echo $parcf ?></strong> (%<?php echo $amountpercentpartialpaid ?>)</span></div>',
				'</div>',
				'</div>',
				...
				'<div class="percentage">%<?php echo $amountpercentduepaid ?></div>',
				'</div>',
				'<div class="box-foot">',
				'<span class="arrow arrow-up"></span>',
				...
				'<div class="box-foot-right"><br><span class="box-foot-stats"><strong><?php echo $vdf ?></strong> (%<?php echo $amountpercentduepaid ?>)</span></div>',
				'</div>',
				'</div>'
				].join('');
			}
