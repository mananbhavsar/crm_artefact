function Workshift_Controller($scope, $http, $mdSidenav, $mdDialog, $filter) {
	"use strict";
...
function buildToggler1(navID) {
	...
	$scope.saving = false;
	$http.get(BASE_URL + 'workshift/get_all_shifts/' + type).then(function (Staff) {
...
CiuisCRM.controller('Workshift_Controller', Workshift_Controller);
