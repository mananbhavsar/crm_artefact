function Customers_Controller($scope, $http, $mdSidenav, $filter, $mdDialog, fileUpload, $q) {
	...
	$scope.selectSaleTeam=function(sale){
		console.log(sale);
		$scope.customer.sales_team='';
		$(".sales_team_opt").removeAttr('disabled');
		$(".sales_team_opt").show();
		$(".sales_team_opt[value='"+sale+"']").hide();
	}
	...
	$scope.statuss = [
	    ...
	    { id: 'Black Listed', name: 'Black Listed' },
					...
					point:{
						events:{
							click: function (event) {
							  $scope.call('Filter by Group',this);
							}
						}
					}          
				}
...
//console.log(imgname);
			...
			if (!$scope.customer) {
				...
				if ($scope.customer.trade_expiry_date) {
					$scope.customer.trade_expiry_date = moment($scope.customer.trade_expiry_date).format("YYYY-MM-DD")
				}
				var dataObj = $.param({
					...
					statuss:'',
					sales_team:$scope.customer.sales_team,
					is_individual:$scope.customer.is_individual,
					main_sales_person:$scope.customer.main_sales_person,
					company_address: $scope.customer.company_address,
					contact_number_office: $scope.customer.contact_number_office,
					account_contact_number: $scope.customer.account_contact_number,
					licence_no: $scope.customer.licence_no,
					trade_expiry_date: $scope.customer.trade_expiry_date,
					//contactname:$scope.customer.contactname,
					//contactemail:$scope.customer.contactemail,
					img_name:imgname,
					creditperiod: $scope.customer.creditperiod,
					creditlimit: $scope.customer.creditlimit,
					terms_and_conditions: $scope.customer.terms_and_conditions,
		...
		$scope.call = function (_prop,value) {
			console.log(value.name);
			for(var key in $scope.filter[_prop]) {
				$scope.filter[_prop][key] = false;
			}
			$scope.filter[_prop][value.name] = true;
		}
...
function Customer_Controller($scope, $http, $filter, $mdSidenav, $mdDialog) {
	...
	$scope.switchcheck=function(is_individual,sale){
		$(".sales_team_opt").show();
		$(".sales_team_opt[value='"+sale+"']").hide();
	...
	$http.get(BASE_URL + 'customers/saleswisedata').then(function (SalesTeam) {
			$scope.salesteam = SalesTeam.data;
		});
	...
	$scope.GetCustomerContacts = function(id) {
		/*$http.get(BASE_URL + 'customers/get_customer/' + id).then(function (Customer) {
			$scope.newcontact.email = Customer.data.email;
			$scope.newcontact.surname = Customer.data.namesurname;
			$scope.newcontact.name = Customer.data.contactpersonname;
		});*/
	}
