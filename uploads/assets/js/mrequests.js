function Mrequests_Controller($scope, $http, $mdSidenav, fileUpload, $filter, $mdToast, $mdDialog) {
	"use strict";
	...
	$scope.ViewFile = function(filename, index, fileid) {
		$scope.fullpath = BASE_URL+'uploads/files/materialrequests/'+index+'/'+filename;
		$scope.fileid = fileid;
		$scope.filename = filename;
		$scope.folderid = index;
		$mdDialog.show({
			templateUrl: 'view_image.html',
			scope: $scope,
			preserveScope: true,
			targetEvent: filename,
		});
	}
	
	$scope.DeleteFile = function(folderid, fileid, filename) {
			...
			$mdDialog.show(confirm).then(function () {
				...
				$http.post(BASE_URL + 'mrequests/delete_mreq_file/' + folderid + '/'+fileid +'/'+filename, config)
					.then(
						function (response) {
	...
	$scope.createTicket = function() {
		...
		var uploadUrl = BASE_URL+'mrequests/create/';
	...
	$scope.ticketsLoader = true;
	$http.get(BASE_URL + 'mrequests/requests').then(function (Tickets) {
...
function Mrequest_Controller($scope, $http, $mdDialog, fileUpload, $mdSidenav) {
	"use strict";
...
CiuisCRM.controller('Mrequests_Controller', Mrequests_Controller);
CiuisCRM.controller('Mrequest_Controller', Mrequest_Controller);
