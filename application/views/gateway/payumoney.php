...
//       $hash_string .= '|';
//     }
//     $hash_string .= $payment['payu_money_salt'];
//     $hash = strtolower(hash('sha512', $hash_string));




// $SALT = $payment['payu_money_salt'];
...
<script>
    ...
    function submitPayumoneyForm() {
      if(hash == '') {
        return;
      }
      var payuForm = document.forms.payumoneyForm;
      console.log('<?php echo $hash ?>');
      payuForm.submit();
    }
  </script>
<body onload="//submitPayumoneyForm()">
	...
	<div id="authorize_gateway">
		<div class="container">
			<div id="Checkout" class="inline">
				<h1>Pay Invoice via 
					<img src="<?php echo base_url('assets/img/payment-modes/payumoney.jpg')?>"></h1>
				<div class="card-row"> <span class="visa"></span> <span class="mastercard"></span> <span class="amex"></span> <span class="discover"></span> </div>
				<form method="post" action="<?php echo $payu['url'];?>"  name="payumoneyForm">
					<input name="amount" value="<?php echo $invoice['total'] ?>" type="hidden">
					<input type="hidden" name="key" value="<?php echo $payment['input_value1'] ?>"/>
			        <input type="hidden" name="invoice_id" value="<?php echo $invoice['id'] ?>"/>
			        <input type="hidden" name="hash" value="<?php echo $hash ?>"/>
			        <input type="hidden" name="txnid" value="<?php echo $txnid ?>"/>
			        <input type="hidden" name="surl"
			               value="<?php echo site_url('gateway/payumoney/invPaymentSuccess'); ?>"/>
			        <input type="hidden" name="furl"
			               value="<?php echo site_url('gateway/payumoney/invPaymentFailure'); ?>"/>
			        <input type="hidden" name="service_provider" value="" size="64"/>


					<input type="hidden" class="form-control" id="name" name="name" placeholder="Enter First Name" value="<?php if($customer['type']===1){echo $customer['namesurname'];} else echo $invoice['company']; ?>">
					...
					<input type="hidden" class="form-control" id="amount" name="amount" placeholder="Enter Amount" value="<?php echo $invoice['total']?>">
					<div class="form-group">
						<label for="PaymentAmount">Payment amount</label>
						<div class="amount-placeholder"> <span><?php echo $currency?></span>
							<span>
					...
					<div class="form-group">
						...
						<input readonly="" required="" id="productinfo" name="productinfo" class="form-control" type="text" maxlength="500" value="Invoice Payment">
					</div>
					<?php //if(!$hash) { ?>
					<button id="PayButton" class="btn btn-block btn-success submit-button" type="submit"> <span class="submit-button-lock"></span> <span class="align-middle">Pay <?php echo $currency?> <?php echo $invoice['total']?></span> </button>
				<?php //} ?>
				</form>
				<ul class="card-types">
					<li class="right"><span> Payments powered by <img class="payulogo" src="<?php echo base_url('assets/img/payment-modes/payulogo.png')?>" alt=""></span></li>
				</ul>
			</div>
		</div>
	</div>
