    ...
    <div id="authorize_gateway">
    	<div class="container">
    		<div id="Checkout" class="inline">
				<h1>Pay Invoice via Razorpay</h1>
				...
				<div class="form-group">
					...
					<input readonly="" required="" id="productinfo" name="productinfo" class="form-control" type="text" maxlength="500" value="Invoice <?php echo $invoice['inv_id'] ?> Payment">
				</div>
				<?php
			        // if (!empty($_POST)) {
			            $form = '<form style="text-align:center" name="redirect" action="' . site_url('gateway/pay_razorpay') . '" method="POST">'; ?>
			            <?php $form .= '<script
			            src="https://checkout.razorpay.com/v1/checkout.js"
					    data-key="'.$payment['input_value1'].'"
					    data-amount="' . ($invoice['total']*100) . '"
					    data-buttontext="Pay with Razorpay"
					    data-name="'.$crm_name.'" 
					    data-description="Invoice ' . $invoice['inv_id'] . ' payment via Stripe " ;
					    data-image="'.$logo.'" 
					    data-prefill.phone = "'.$customer['phone'].'"
					    data-prefill.contact = "'.$customer['phone'].'"
					    data-prefill.name="'.$cust_name.'"
					    data-prefill.email="'.$customer['email'].'"
					    data-theme.color="#0450cb"
					    data-currency="' . $currency . '">
			        </script>
			        ' . form_hidden('invoice_id', $invoice['id']) . '
			        ' . form_hidden('amount', $invoice['total']) . '
			    ...
			    <input name="amount" id="amount" value="<?= $invoice['total'] ?>" type="hidden">
			...
			<ul class="card-types">
				<li class="right"><span> Payments powered by 
					<img style="height: 20px;" src="<?php echo base_url('assets/img/payment-modes/razorpay.png')?>" alt=""></span>
				</li>
			</ul>
		</div>
	</div>
