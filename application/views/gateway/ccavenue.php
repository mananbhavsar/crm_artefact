	...
	<div id="authorize_gateway">
		<div class="container">
			<div id="Checkout" class="inline">
				<h1>Pay Invoice via CCAvenue Checkout</h1>
				<div class="card-row"> <span class="visa"></span> <span class="mastercard"></span> <span class="amex"></span> <span class="discover"></span> </div>
				<form method="post" action="<?php echo base_url('gateway/ccavenue_confirm');?>"  name="">
					<input name="amount" value="<?php echo $invoice['total'] ?>" type="hidden">
					<input type="hidden" name="tid" id="tid" value="<?php echo $txnid ?>"/>
            		<input type="hidden" name="merchant_id" value="<?php echo $payment['input_value1'] ?>"/>
            		<input type="hidden" name="redirect_url" value="<?php echo site_url('gateway/ccavenue_success'); ?>"/>
	            	<input type="hidden" name="cancel_url" value="<?php echo site_url('gateway/ccavenue_cancel'); ?>"/>
	            	<input type="hidden" name="language" value="EN"/>
	            	<input type="hidden" name="billing_name" class="form-control" value="<?php if($customer['type']===1){echo $customer['namesurname'];} else echo $invoice['company']; ?>">
	                <input type="hidden" name="billing_email" class="form-control" value="<?php echo $customer['email'] ?>">
	                <input type="hidden" name="billing_tel" class="form-control" value="<?php echo $customer['phone'] ?>">
	                <input type="hidden" value="<?php echo $customer['address'] ?>" name="billing_address">
	                <input type="hidden" name="billing_city" value="<?php echo $customer['city'] ?>" class="form-control">
	                <input type="hidden" name="billing_state" value="<?php echo $customer['state'] ?>" class="form-control">
	                <input type="hidden" name="billing_zip" value="<?php echo $customer['zipcode'] ?>" class="form-control">
	                <input type="hidden" name="billing_country" value="<?php echo $customer['country_id'] ?>"> 
					...
					<button id="PayButton" class="btn btn-block btn-success submit-button" type="submit"> 
						...
						<span class="align-middle">Pay <?php echo $currency?> <?php echo $invoice['total']?></span> 
					</button>
				</form>
				<ul class="card-types">
					<li class="right"><span> Payments powered by 
						<img style="height: 20px;" src="<?php echo base_url('assets/img/payment-modes/ccavenue.png')?>" alt=""></span>
					</li>
				</ul>
			</div>
		</div>
