...
<div class="ciuis-body-content" ng-controller="Calendar_Controller">
        ...
        <md-tabs md-dynamic-height md-border-bottom>
          ...
          <md-tab label="<?php echo lang('Holidays')?>">
            <md-content class="events events_xs calendar-events" style="margin-top: 0px">
              <md-toolbar class="toolbar-white" style="border-bottom:unset">
                <div class="md-toolbar-tools">
                  <h3 flex md-truncate><?php echo 'Holidays' ?></h3>
              ...
              <ul style="padding: 0px 20px 0px 20px;" ng-cloak>
				<li ng-repeat="event in all_holidays | pagination : currentPage*itemsPerPage | limitTo: 5">
					<div class="row">
						<div class="col-sm-12">
							<div class="col-sm-8">
								<label class="date" ng-show="event.event_type1==1"> <span class="weekday" ng-bind="event.day"></span><span class="day" ng-bind="event.aday"></span> </label>
								<b><h3 ng-bind="event.title"></h3></b>
								<p  ng-show="event.event_type1==1">
									<span class='duration' ng-bind="event.start_date | date : 'MMM d, y'"></span>
									<span class='duration' ng-bind="event.end_date | date : 'MMM d, y'"></span>
									<!--- <span class='location' ng-bind="event.staff"></span>-->
								</p>
								<p  ng-show="event.event_type1==2">
								<span class='duration' ng-bind="event.normal_holidays" style="border-right:0px;"></span>
							  </p>
							</div>
							<div class="col-sm-4">
								<md-icon ng-click="EditHoliday(event.id,event.title,event.event_type1,event.dateType,event.start,event.end,event.normal_holidays1,event.detail, $event)" class="md-secondary md-hue-3 ion-compose " aria-hidden="Edit Holiday"></md-icon>
								<md-icon ng-click='DeleteHoliday($index)' aria-label="Remove Holiday" class="md-secondary md-hue-3 ion-trash-b" ng-show="event.event_type1==1"></md-icon>
							</div>
						</div>
					</div>
				</li>
              ...
              <md-content ng-show="!all_holidays.length" class="text-center bg-white" ng-cloak>
                <h1 class="text-success"><i class="mdi mdi-calendar-check"></i></h1>
                <span class="text-muted"><?php echo lang('no_holiday_today') ?></span>
              </md-content>
            </md-content>
			 <div ng-show="all_holidays.length>6 && !documentFiles" class="pagination-div" ng-cloak>
				<ul class="pagination">
              ...
              <md-content class="bg-white">
                ...
                <div>
                  ...
                  ng-style="{background:task_color}" color-picker-position="bottom" />
				   ...
				   <md-input-container class="md-block">
                    <label><?php echo lang('pickacolor') . ' ' . lang('holiday')?></label>
                    <input color-picker color-picker-model="holiday_color" ng-model="holiday_color"
                  ng-style="{background:holiday_color}" color-picker-position="bottom" />
                  </md-input-container>
                </div>
              </md-content>
            </md-tab-body>
  ...
  <div id="fullCalModaltask" class="modal fade"> 
   ...
   <div id="fullCalModalholiday" class="modal fade"> 
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header" style="border-bottom: 1px solid #d8d8d8;">
          <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span> 
        ...
        <div id="modalBody" class="modal-body">
          ...
          <br><br>
          <p>
            <b><?php echo lang('summary') ?>: </b> <span id="holidaydescription"></span>
          </p>
  ...
  <md-sidenav class="md-sidenav-right md-whiteframe-4dp eventFormToggle" md-component-id="HolidayForm" ng-cloak style="width: 450px;">
    ...
    <md-content layout-padding="">
      <md-content layout-padding>
		...
		<div layout="row" layout-wrap  ng-show="eventType==1">
			 <div class="form-group">
				  <label class="radio-inline"><input type="radio" name="one_day"  value="1"  ng-model="dateType">One Day</label>
				<label class="radio-inline"><input type="radio" name="more_then_one" value="2"  ng-model="dateType">More than one</label>
			</div>
		</div>
        <div layout="row" layout-wrap >
          <div flex-gt-xs="50" flex-xs="50" ng-show="dateType==1 || dateType==2">
            <md-input-container class="md-block">
              <label><?php echo lang('holiday') . ' ' . lang('start') ?></label>
              <input mdc-datetime-picker="" date="true" time="false" type="text" id="datetime1"  minutes="true" min-date="date" placeholder="<?php echo lang('holiday') . ' ' . lang('start') ?>"  show-icon="true" ng-model="event_start1" ng-change="getstartdate(event_start1)"class=" dtp-no-msclear dtp-input md-input">
            </md-input-container>
          </div>
          <div flex-gt-xs="50" flex-xs="50" ng-show="dateType==2">
            <md-input-container class="md-block">
              <label><?php echo lang('holiday') . ' ' . lang('end') ?></label>
              <input mdc-datetime-picker="" date="true" time="false" type="text" id="datetime2" minutes="true" min-date="startdate_new" show-icon="true" ng-model="event_end" class="dtp-no-msclear dtp-input md-input" placeholder="<?php echo lang('event') . ' ' . lang('end') ?>">
            </md-input-container>
          </div>
        </div>
		<div layout="row" layout-wrap ng-show="eventType==2">
          <div flex-gt-xs="90" flex-xs="90">
            <md-input-container class="md-block">
              <label><?php echo lang('days'); ?></label>
              <md-select placeholder="<?php echo lang('days'); ?>"  ng-model="normal_holidays" style="min-width: 200px;" id="normal_holidays" ng-change="changed(normal_holidays)" multiple>
					<md-option ng-repeat="week in weekdays" value="week.name" ng-value="week.name">{{week.name}}</md-option>
              </md-select>
            </md-input-container><br>
          </div>
        ...
        <div>
          <md-button ng-click="AddHoliday()" class="md-raised md-primary btn-report block-button" aria-label='Add Holiday' ng-disabled="addingHoliday == true" ng-show="event_id==0">
		  ...
		  <md-button ng-click="AddHoliday()" class="md-raised md-primary btn-report block-button" aria-label='Add Holiday' ng-disabled="addingHoliday == true" ng-show="event_id!=0">
            <span ng-show="addingHoliday == false"><?php echo lang('update') . ' ' . lang('holiday') ?></span>
            <md-progress-circular class="white" ng-show="addingHoliday == true" md-mode="indeterminate" md-diameter="20">
            </md-progress-circular>
          </md-button>
          <br/><br/><br/><br/>
 ...
 <!---<button id="openEventModals" style="display: none;" ng-click="EventForm()"></button>--->
  ...
  <script>
    ...
    var load_more = "<?php echo lang('load_more');?>";
	 var holiday_color =  "<?php echo $appconfig['holiday_color'];?>";
  </script>
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      eventClick: function(event, jsEvent, view) {
        ...
        }else if (events.relation == 'holiday') {
          $('.event_type').html(event.event._def.title);
          $('.startdate').text(events.start_date);
          $('.enddate').text(events.end_date);
          $('#holidaydescription').html(events.detail);
          $('#fullCalModalholiday').modal();
        }
      },
      selectable: true,
     /*  select: function(start, end, jsEvent, view) {
	  ...
	  {
        url: '<?php echo base_url('api/holidays1');?>',
      },
      ],
