...
<div class="ciuis-body-content" ng-controller="Mrequests_Controller">
        ...
        <div class="col-md-3 col-xs-6 border-right text-uppercase">
          <div class="tasks-status-stat">
            <h4 class="text-bold ciuis-task-stat-title">
              <span class="task-stat-number ng-binding" ng-bind="(tickets | filter:{status:'0'}).length"></span>
              <span class="task-stat-all ng-binding" ng-bind="'/'+' '+tickets.length+' '+'<?php echo 'Open' ?>'"></span>
            </h4>
            <span class="ciuis-task-percent-bg">
              <span class="ciuis-task-percent-fg" style="width: {{(tickets | filter:{status:'0'}).length * 100 / tickets.length }}%;"></span>
            </span>
          </div>
        ...
        <div class="col-md-3 col-xs-6 border-right text-uppercase">
          <div class="tasks-status-stat">
            <h4 class="text-bold ciuis-task-stat-title">
              ...
              <span class="task-stat-all ng-binding" ng-bind="'/'+' '+tickets.length+' '+'<?php echo 'Pending' ?>'"></span>
            </h4>
            <span class="ciuis-task-percent-bg">
              <span class="ciuis-task-percent-fg" style="width: {{(tickets | filter:{status:'1'}).length * 100 / tickets.length }}%;"></span>
            </span>
          </div>
          <span style="color:#989898"><?php //echo 'Pending' ?></span>
        </div>
        <div class="col-md-3 col-xs-6 border-right text-uppercase">
          <div class="tasks-status-stat">
            <h4 class="text-bold ciuis-task-stat-title">
              <span class="task-stat-number ng-binding" ng-bind="(tickets | filter:{status:'2'}).length"></span>
          ...
          <span style="color:#989898"><?php echo 'Approved' ?></span>
        </div>
        <div class="col-md-3 col-xs-6 border-right text-uppercase">
          <div class="tasks-status-stat">
            <h4 class="text-bold ciuis-task-stat-title">
              <span class="task-stat-number ng-binding" ng-bind="(tickets | filter:{status:'3'}).length"></span>
              <span class="task-stat-all ng-binding" ng-bind="'/'+' '+tickets.length+' '+'<?php //echo lang('ticket') ?>'"></span>
            </h4>
            <span class="ciuis-task-percent-bg">
              <span class="ciuis-task-percent-fg" style="width: {{(tickets | filter:{status:'3'}).length * 100 / tickets.length }}%;"></span>
            </span>
          </div>
          <span style="color:#989898"><?php echo 'Declined' ?></span>
        </div>
          ...
          <div class="col-md-12 ticket-row-left text-left">
            <div class="tickets-vertical-menu">
              ...
              <a ng-click="TicketsFilter = {request_type: 4}" class="side-tickets-menu-item"><?php echo 'Salary Request' ?> <span class="ticket-num" ng-bind="(tickets | filter:{request_type:'4'}).length"></span></a>
			   ...
			   <a ng-click="TicketsFilter = {status: 0}" class="side-tickets-menu-item"><?php echo 'Open' ?> <span class="ticket-num" ng-bind="(tickets | filter:{status:'0'}).length"></span></a>
              <a ng-click="TicketsFilter = {status: 2}" class="side-tickets-menu-item"><?php echo 'Approved' ?> <span class="ticket-num" ng-bind="(tickets | filter:{status:'2'}).length"></span></a>
              <a ng-click="TicketsFilter = {status: 1}" class="side-tickets-menu-item"><?php echo 'Pending' ?> <span class="ticket-num" ng-bind="(tickets | filter:{status:'1'}).length"></span></a>
  ...
  <div class="main-content container-fluid col-xs-12 col-md-9 col-lg-9 md-p-0 lead-table">
   ...
   <select name="statuscha"  onchange="changestatus(this.value)" class="form-control" id="" style="width: 132px;">

   <option value="{{ticket.id}}id0" id="0" ng-selected="ticket.status=='0'" ng-style="myOptionStyle();"><?php echo 'Open'; ?></option>
				<option value="{{ticket.id}}id1"  id="1" ng-selected="ticket.status=='1'" ><?php echo 'Pending'; ?></option>
				<option value="{{ticket.id}}id2"   id="2" ng-selected="ticket.status=='2'"><?php echo 'Approved'; ?></option>
				<option value="{{ticket.id}}id3"  id="3" ng-selected="ticket.status=='3'"><?php echo 'Declined'; ?></option></select>
</div>
			  ...
			  <td md-cell>
              <md-card-title-media > 
            <img src="<?php echo base_url('uploads/images/{{ticket.staffavatar}}'); ?>" alt="Avatar"  class="md-avatar1" style="max-height: 36px;height: 100%;max-width: 40px;"></md-card-title-media>
...
function show_post(image,id)
  {
     $.ajax({
              url : "<?php echo base_url(); ?>mrequests/img/"+id,
...
function changestatus(str){
	...
	$.ajax({
        url: '<?php print base_url();?>mrequests/update_status/'+str+'',
        type: 'GET',
