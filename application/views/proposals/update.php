  ...
  <div ng-controller="Proposal_Controller" class="main-content container-fluid col-xs-12 col-md-12 col-lg-9"> <?php echo form_open('proposals/update',array("class"=>"form-horizontal proposalForm")); ?>
    <md-toolbar class="toolbar-white">
      <div class="md-toolbar-tools">
        ...
        <md-switch ng-show="proposal.is_requested == '0'" ng-model="proposal.proposal_type" aria-label="Type" ng-cloak><strong class="text-muted"><?php echo lang('for_lead')?></strong></md-switch>
        ...
        <md-button type="button" ng-click="saveAll()" class="md-icon-button" aria-label="Save" ng-cloak>
          ...
          <md-tooltip ng-hide="savingProposal == true" md-direction="bottom"><?php echo lang('save') ?></md-tooltip>
    ...
    <md-content class="bg-white" layout-padding ng-cloak>
      <div layout-gt-xs="row">
        ...
        <md-input-container ng-show="proposal.proposal_type == false" class="md-block" flex-gt-xs>
          ...
          <md-select required placeholder="<?php echo lang('choisecustomer'); ?>" ng-model="proposal.customer" name="customer" style="min-width: 200px;" data-md-container-class="selectdemoSelectHeader">
      ...
      <div layout-gt-xs="row">
        ...
        <md-input-container class="md-block" flex-gt-xs>
          ...
          <md-select ng-init="statuses = [{id: 0,name: '<?php echo lang( 'quote' ).' '.lang( 'request' ); ?>'},{id: 1,name: '<?php echo lang('draft'); ?>'}, {id: 2,name: '<?php echo lang('sent'); ?>'}, {id: 3,name: '<?php echo lang('open'); ?>'}, {id: 4,name: '<?php echo lang('revised'); ?>'}, {id:5,name: '<?php echo lang('declined'); ?>'}, {id: 6,name: '<?php echo lang('accepted'); ?>'}];" required placeholder="<?php echo lang('status'); ?>" ng-model="proposal.status" name="status" style="min-width: 200px;">
      ...
      <div layout-gt-xs="row">
        ...
        <md-input-container disabled ng-show="proposal.is_requested == '1'" class="md-block" flex-gt-xs>
          <label><?php echo lang( 'requested' ).' '.lang( 'quote' ).' '.lang('details') ?></label>
          <textarea disabled ng-model="proposal.customer_quote" rows="3"></textarea>
        </md-input-container>
      </div>
      <md-checkbox class="pull-right" ng-model="proposal.comment" ng-true-value="true" ng-false-value="false" aria-label="Comment"> <strong class="text-muted text-uppercase"><?php echo lang('allowcomments');?></strong> </md-checkbox>
    </md-content>
    <md-content class="bg-white" layout-padding ng-cloak>
