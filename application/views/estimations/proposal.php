...
<div class="main-content container-fluid col-md-9 borderten">
    <md-toolbar class="toolbar-white">
      <div class="md-toolbar-tools">
        ...
        <h2 flex md-truncate><?php echo $estimation_record['estimate_status'] != 'Quote Request' ? 'EST'.$estimation_record['estimation_id'] : 'QR';?> <?php echo $estimation_record['project_name'];?></h2>
...
<md-menu-item>
              ...
              <?php } ?>
				...
				<?php if($estimation_record['estimate_status']=='Draft' && $userInfo['role_name']=='Estimator' && $estimation_record['estimation_total_amt'] > 0){ ?> 
					<md-menu-item>
					  <md-button ng-click="MarkAs('Under Approval','Under Approval')"  aria-label="Under Approval">Send For Approval</md-button>
					</md-menu-item>	
				<?php }?>
				
				<?php if(($estimation_record['estimate_status']=='Under Approval' || $estimation_record['estimate_status']=='Draft') && $estimation_record['showAccess']== '1' && $estimation_record['estimation_total_amt'] > 0){ ?> 
					<md-menu-item>
					  <md-button ng-click="MarkAs('Approved','Approved')"  aria-label="Mark As Approved">Mark As Approved</md-button>
					</md-menu-item>
					<md-menu-item>
					  <md-button ng-click="estStatusDeclined('Declined')"  aria-label="Declined">Mark As  Declined
					  </md-button>
					</md-menu-item>				
				<?php } ?>
        ...
        <md-tab label="<?php echo 'Estimation'; ?>">
          <md-content class="md-padding bg-white">
            <div class="proposal">
			 ...
			 <div id="details" class="clearfix">
                  ...
                  <div id="client">
					...
					<div class="to"><label><?php echo 'Attn'; ?>:</label>
					 <?php echo $client_contact_person['name'] ?>
					</div>
                  </div>
                  <div id="invoice">
					...
					<?php echo $estimation_record['estimate_status'];?>
					<?php if($estimation_record['estimate_status'] == 'Declined' && !empty($declined_notes)) { ?>
				...
				<?php 
				 $disshowhead=0;
				foreach($estimation_main_items as $k => $main_items ) {
				     
				  if($main_items['item_discount']!=0){
				       $disshowhead++;
				  }
				}
                ...
                <table border="0" cellspacing="0" cellpadding="0">
				  ...
				  $totalitemprice+=$main_items['item_total_price'];
                  ...
                  <tbody>
                    <tr>
                      <td class="desc">
						...
						<p class="pre_view ng-binding"><?php echo $main_items['item_description'];?></p>
                        </td>
                      </td>
                      ...
                      <?php if($disshowhead>0){?>
                       <td class=" unit text-right"><?php echo $main_items['item_discount'];?></td>
                       <?php }?>
                      <td class="total text-right"><?php echo number_format($main_items['item_total_price'],2, '.', ','); ?></td>
			...
			<div class="col-md-6 row">
			      <div class="col-md-12">
				  <div class="col-md-10">
                    <div class="text-right text-uppercase text-muted">Total:</div></div><div class="col-md-2 text-right"><?php echo number_format($totalitemprice,2, '.', ',');?></div></div>
				  ...
				  <div class="col-md-10">
					...
					<?php }?>
					   ...
					   <?php if( $disshowhead>0 || $estimation_record['discount']!=0){?>
                  <div class="col-md-12">
				  <div class="col-md-10">
                    <div class="text-right text-uppercase text-muted">Net Total:</div></div><div class="col-md-2 text-right"><?php echo number_format($estimation_record['subtotal_amt'],2, '.', ',');?></div></div>
                    <?php }?>
					  <div class="col-md-12">
				  <div class="col-md-10">
                    <div  class="text-right text-uppercase text-muted">
				...
				<hr>
				<h3>Description</h3>
				<div class="special_notes"><?php echo $description; ?></div>
			 </main>
             </div>
          </md-content>
...
<md-toolbar class="toolbar-white">
	...
	<div class="col-md-12 col-xs-12 md-pr-0 md-pl-0 md-pb-10" style="background: white">
      <div class="col-xs-12 task-sidebar-item" ng-cloak>
        <ul class="list-inline task-dates">
          <li class="col-md-6 col-xs-6">
			...
			<?php if(sizeof($orderResult) > 0  && $estimation_record['estimate_status'] =='Approved'){?>
				<strong class="text-bold"><a class="label label-info" href="<?php echo base_url('orders/order/'.$orderResult['orderid'].'')?>">Quotes <i class="ion-android-open"></i></a></strong>
			<?php }else{ ?>
				<strong class="text-bold"><a class="label label-info" href="<?php echo base_url('estimations/update/'.$pid.'')?>">Estimations <i class="ion-android-open"></i></a></strong>
			<?php } ?>
		  </li>
          <li class="col-md-6 col-xs-6">
	 ...
	 <?php if(sizeof($orderResult) > 0){?>
		  <div class="col-xs-12 task-sidebar-item" ng-cloak>
			<ul class="list-inline task-dates">
			  <li class="col-md-6 col-xs-6">
				<h5>Requested By</h5>
				<span class="mdi mdi-assignment-account"></span> <strong ><?php echo $orderResult['requested_by'];?></strong> </li>
			  <li class="col-md-6 col-xs-6">
				<h5>Requested Date</h5>
				<strong><?php echo date('d-m-Y h:i A',strtotime($orderResult['requested_on']));?></strong>
			  </li>
			</ul>
		  </div>
		  <?php if($estimateByStaff){?>
		  <div class="col-xs-12 task-sidebar-item" ng-cloak>
			<ul class="list-inline task-dates">
			  <li class="col-md-6 col-xs-6">
				<h5>Estimated By</h5>
				<span class="mdi mdi-assignment-account"></span> <strong ><?php echo $estimateByStaff;?></strong> </li>
			  <li class="col-md-6 col-xs-6">
				<h5>Estimated Date</h5>
				<strong><?php echo $estimateDate;?></strong>
			  </li>
			</ul>
		  </div>
		  <?php } } else { ?>
	 <div class="col-xs-12 task-sidebar-item" ng-cloak>
        <ul class="list-inline task-dates">
          ...
          <li class="col-md-6 col-xs-6">
            <h5>Estimated Date</h5>
			<strong><?php print date('d-m-Y H:i A',strtotime($estimation_record['created']));?></strong>
			<!--
      ...
      <div class="col-xs-12 task-sidebar-item" ng-cloak>
        <ul class="list-inline task-dates">
          ...
          <li class="col-md-6 col-xs-6">
            <h5>Approved Date</h5>
			<strong><?php echo date('d-m-Y h:i A',strtotime($estimation_record['approved_date']));?></strong>
			<!--
            <strong ng-bind="task.created"></strong>--> </li>
        </ul>
      </div>
...
<script type="text/ng-template" id="declined-template.html">
  <md-dialog aria-label="options dialog">
  <?php echo form_open_multipart('estimations/add_declined_msg/',array("class"=>"form-horizontal")); ?>
  <md-dialog-content layout-padding>
	...
	<md-input-container class="md-block" style="margin-top: 20px !important;">
		<textarea class="min_input_width" ng-model="estdeclined_msg" name="estdeclined_msg"  id="estdeclined_msg"></textarea>
	</md-input-container> 
  </md-dialog-content>
  <md-dialog-actions>
    <span flex></span>
    <md-button ng-click="closemarkas()" aria-label="add"><?php echo lang('cancel') ?>!</md-button>
    <md-button ng-click="estDeclined()" class="template-button" ng-disabled="uploading == true">
