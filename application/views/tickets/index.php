...
<div class="ciuis-body-content" ng-controller="Tickets_Controller">
  ...
  <div class="main-content container-fluid col-xs-12 col-md-3 col-lg-3 md-pl-0">
    <div class="panel-default panel-table borderten lead-manager-head">
      <md-content style="border-bottom: 2px dashed #e8e8e8; padding-bottom: 20px;display:none;" layout-padding>
        ...
        <div id="tickets-left-column text-left">
          <div class="col-md-12 ticket-row-left text-left">
            <div class="tickets-vertical-menu">
              ...
              <a ng-click="TicketsFilter = {status_id: 1};ActiveChangeColor('open');" class="side-tickets-menu-item" id="ticket_open"><?php echo lang('open') ?> <span class="ticket-num open" ng-bind="(tickets | filter:{status_id:'1'}).length"></span></a>
              <a ng-click="TicketsFilter = {status_id: 2};ActiveChangeColor('inprogress');" id="ticket_inprogress" class="side-tickets-menu-item"><?php echo lang('inprogress') ?> <span class="ticket-num" ng-bind="(tickets | filter:{status_id:'2'}).length"></span></a>
              <a ng-click="TicketsFilter = {status_id: 3};ActiveChangeColor('answered');" id="ticket_answered" class="side-tickets-menu-item"><?php echo lang('answered') ?> <span class="ticket-num" ng-bind="(tickets | filter:{status_id:'3'}).length"></span></a>
              <a ng-click="TicketsFilter = {status_id: 4};ActiveChangeColor('answered');" id="ticket_closed" class="side-tickets-menu-item"><?php echo lang('closed') ?> <span class="ticket-num" ng-bind="(tickets | filter:{status_id:'4'}).length"></span></a>
              ...
              <a ng-click="TicketsFilter = {priority_id: 1};ActiveChangeColor('low');" id="ticket_low" class="side-tickets-menu-item"><?php echo lang('low') ?> <span class="ticket-num" ng-bind="(tickets | filter:{priority_id:'1'}).length"></span></a>
              <a ng-click="TicketsFilter = {priority_id: 2};ActiveChangeColor('medium');" id="ticket_medium" class="side-tickets-menu-item"><?php echo lang('medium') ?> <span class="ticket-num" ng-bind="(tickets | filter:{priority_id:'2'}).length"></span></a>
              <a ng-click="TicketsFilter = {priority_id: 3};ActiveChangeColor('high');" id="ticket_high" class="side-tickets-menu-item"><?php echo lang('high') ?> <span class="ticket-num" ng-bind="(tickets | filter:{priority_id:'3'}).length"></span></a>
            </div>
          </div>
        </div>
      </div>
   ...
   <md-content layout-padding="">
    <md-content>
     ...
     <md-input-container class="md-block" flex-gt-xs>
        ...
        <md-select ng-init="priorities = [{id: 1,name: '<?php echo lang('low'); ?>'}, {id: 2,name: '<?php echo lang('medium'); ?>'}, {id: 3,name: '<?php echo lang('high'); ?>'}, {id: 4,name: '<?php echo lang('duedate'); ?>'}];" required placeholder="<?php echo lang('priority'); ?>" ng-model="ticket.priority" ng-change="Getdatepicker(ticket.priority);" name="priority">
          <md-option ng-value="priority.id" ng-repeat="priority in priorities"><span class="text-uppercase">{{priority.name}}</span></md-option>
        </md-select>
      </md-input-container>
      <div class="md-block" ng-if="ticket.priority == 4">
	  <br>
          <label><?php echo lang('duedate') ?></label>
			<div class="input-group">
				<input type="text" name="due_date" class="form-control " id="due_date" ng-model="ticket.due_date" ><span class="input-group-addon"><i class="glyphicon glyphicon-th"></i></span>
			</div> 
