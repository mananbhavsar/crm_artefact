...
class Document_Model extends CI_Model {
	...
	function get_document_doc( $id ) {
		return $this->db->get_where( 'uploads_documents', array( 'document_id' => $id ) )->result_array();
	}
	function get_file_new($id) {
		return $this->db->get_where( 'uploads_documents', array( 'upload_document_id' => $id) )->row_array();
	}
...
function get_all_suppliers_material($id) {
	...
	function delete_material($material_id) {
		$this->db->delete('history_logs', array( 'vendor_id' => $material_id,'type'=>'Document'));
    ...
    function update_material($id, $params) {
		...
		$this->db->insert( 'history_logs', array(
			...
			'detail' => ( '<a href="staff/staffmember/' . $this->session->usr_id . '"> ' . $this->session->staffname . '</a> ' . lang( 'updated' ) . ' <a href="document/docview/' .$id . '">' . $params['name'] . '</a>.' ),
	...
	function add_docs( $params ) {
		...
		$this->db->insert( 'history_logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '<a href="staff/staffmember/' . $this->session->usr_id . '"> ' . $this->session->staffname . '</a> ' . lang( 'added' ) . ' <a href="document/docview/' . $document_id . '">' . $params['name'] . '</a>.' ),
			'staff_id' => $loggedinuserid,
			'type'=>'Document',
	...
	function add_document_documents($params){		
	$this->db->insert( 'uploads_documents', $params );	
	$loggedinuserid = $this->session->usr_id;
		$this->db->insert( 'history_logs', array(
			'date' => date( 'Y-m-d H:i:s' ),
			'detail' => ( '<a href="staff/staffmember/' . $this->session->usr_id . '"> ' . $this->session->staffname . '</a> ' . lang( 'Upload Document' ) . ' <a href="document/docview/' .$params['document_id'] . '">' . $params['document_name'] . '</a>.' ),
			'staff_id' => $loggedinuserid,
			'type'=>'Document',
			'vendor_id'=>$params['document_id']
		) );	
	}
