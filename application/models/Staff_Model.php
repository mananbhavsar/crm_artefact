...
class Staff_Model extends CI_Model {
	...
	function get_documents_lable( $id) {
		$sql = "SELECT * FROM staff_doc_heading as stf WHERE staff_id='".$id."' OR staff_id='0'";
	$res = $this->db->query($sql);
	...
	function get_all_staff($type="") {
		...
		$this->db->select( '*,departments.name as department, status.name as statusname,status.color as statuscolor,staff.id as id' );
		$this->db->join( 'departments', 'staff.department_id = departments.id', 'left' );
		$this->db->join( 'status', 'staff.status = status.id', 'left' );
   ...
   $dep_resultname=0;
	...
	function updateappraisal_increment($id, $params ) {
		$this->db->where('id',$id);
		$this->db->update( 'staff_increment', $params );
		return true;
	}
	...
	function update_warning( $params,$warning_id ) {
		$this->db->where('id',$warning_id);
		$this->db->update( 'staff_warning', $params );
		return true;

	}
	function update_tools( $params ,$tools_id) {
		$this->db->where('id',$tools_id);
		$this->db->update( 'staff_tools', $params );
		return true;
   ...
   if($row){
	...
	/////Start Status Setting Code By Namrta///////////////
	function get_all_Status() {
		$this->db->order_by( 'id', 'asc' );
		return $this->db->get_where( 'status', array( '' ) )->result_array();
	}
	...
	function remove_status( $id ) {
		$response = $this->db->delete( 'status', array( 'id' => $id ) );
	}
	/////End Status Setting Code By Namrta///////////////
	
	function update_heading($id, $params) {

			$this->db->where( 'doc_id', $id );
		return $this->db->update( 'staff_doc_heading', $params );
