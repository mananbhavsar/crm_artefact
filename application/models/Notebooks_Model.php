...
use Dompdf\Dompdf;
class Notebooks_Model extends CI_Model {
	...
	{	
		$this->db->select( '*,staff.staffavatar,staff.staffname');
		$this->db->join('staff',' notebooks.created_by=staff.id');
		// $this->db->join('notebook_dec','notebook_dec.notebook_id=notebooks.notebook_id');
		return $this->db->get_where( 'notebooks')->result_array();
	} 
	 function get_page_names($notebook_id) 
	{
	/*	$this->db->select( 'group_concat(notes_title) page_names');
		$this->db->where('notebook_id',$notebook_id);
		$this->db->group_by('notebook_id');

	 return $this->db->get_where( 'notebook_dec')->result_array(); */
	 
	 $sql = "SELECT GROUP_CONCAT(sub.notes_title) AS page_names
FROM 
(
  SELECT notes_title
  FROM notebook_dec
  ...
  LIMIT 3
	...
	function get_notebooks_by_userid($id)
	{	
		return $this->db->get_where( 'notebooks', array( 'created_by' => $id ) )->result_array();
	}
	
   	function get_notebooks_data($id)
	{	
		$this->db->select( '*');
		$this->db->from('notebooks');
		 $this->db->join(' notebook_dec',' notebook_dec.notebook_id=notebooks.notebook_id','left');
		  $this->db->join(' notebook_files','notebook_files.notebook_dec_id=notebook_dec.id','left');
		$this->db->where('notebooks.notebook_id',$id);
		$query=$this->db->get();
	...
	function get_notebooks_id($id)
	{	
		$this->db->select( '*');
		$this->db->where('notebook_id',$id);
		return $this->db->get_where( 'notebooks')->row();
	}
	function get_notebooks_descritpion($id)
	{	
		$this->db->select( '*');
		$this->db->where('notebook_id',$id);
		return $this->db->get_where('notebook_dec')->result();
	}

	function get_notebooks_files($id)
	{	
		$this->db->select( '*');
		$this->db->where('notebook_dec_id',$id);
		return $this->db->get_where('notebook_files')->result();
	}
	...
	function delete_notebooks_desc_id($id)
	{	
		$this->db->where('notebook_id',$id);
		return $this->db->delete( 'notebook_dec');
	}
	...
	function get_materials(){
		$this->db->select( '*' );
		return $this->db->get_where('materials',array())->result_array();
	...
	function get_mat_data($id)
{
	...
	function get_all_notebooks_bysearch($searchKey,$userId='')
	{	
		$userCondition='';
		if($userId !=''){
			$userCondition=' AND notebooks.created_by="'.$userId.'"';
		}else{
			$userCondition='';
		}
		$where = "notebook_dec.notes_title LIKE '%".$searchKey."%' OR notebooks.notebook_list LIKE '%".$searchKey."%'".$userCondition; 
		$this->db->select( 'notebooks.notebook_id, notebooks.notebook_list,notebooks.created_date,notebooks.created_by,notebooks.remarks,notebooks.description');
		$this->db->from('notebooks');
		$this->db->join('notebook_dec',' notebook_dec.notebook_id=notebooks.notebook_id');
		// $this->db->join('staff',' notebooks.created_by=staff.staff_id');
		$this->db->where($where);
		$this->db->group_by('notebooks.notebook_id'); 
		$this->db->order_by('notebooks.notebook_id', 'DESC');
