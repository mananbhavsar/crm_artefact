...
class Events_Model extends CI_Model {
	...
	function get_all_events() {
		...
		//$this->db->where( 'events.public = "1" OR events.staff_id = ' . $this->session->userdata( 'usr_id' ) . '' );
		return $this->db->order_by("start","desc")->get_where( 'holidays' )->result_array();
	}
	...
	function update_holiday($params,$id) {
		$this->db->where( 'id', $id);
		$this->db->update( 'holidays', $params);
		return true;
	}
	function remove( $id ) {
	...
	function remove_holiday( $id ) {
		$response = $this->db->delete( 'holidays', array( 'id' => $id ) );
	}
}