...
//this function gets the currency id from the json file 
	...
	function save_individual_permissions_by_staff($params) {
		$permissions = $this->input->post( 'permissions' );
		$this->addIndividualPermission($permissions, $params['staff_id'],$params['role_id']);
	}
	
	function addIndividualPermission($permission, $staff_id, $role_id){
		foreach($permission as $eachnode){
			$this->saveeachIndividualTreePermission($eachnode, $staff_id, $role_id);
		}	
	}
	
	function saveeachIndividualTreePermission($eachpermission, $staff_id, $role_id) {
			$this->db->insert( 'individual_role_permissions', array(
				'permission_id' => $eachpermission['id'],
				'permission_view_own' => $eachpermission['permission_view_own'] == 'true' ? 1 : 0,
				'permission_view_all' => $eachpermission['permission_view_all'] == 'true' ? 1 : 0,
				'permission_create' => $eachpermission['permission_create'] == 'true' ? 1 : 0,
			...
			if($eachpermission['viewChilds'] == 1){
				$this->addIndividualPermission($eachpermission['child'],$staff_id,$role_id);
			}
	}
