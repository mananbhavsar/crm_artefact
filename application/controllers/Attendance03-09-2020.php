...
class Attendance extends CIUIS_Controller {
	...
	if($path == 3 || $path == 4){
		$data['attend_details'] = $this->Attendance_Model->get_filter_attendance_log($startdate,$enddate);
	}else{
	
	$data['attend_details'] = $this->Attendance_Model->get_current_attendance_log($data['attendance_date']);
	}
	//echo "<pre>";
