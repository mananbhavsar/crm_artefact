...
if($request == 2){
	...
	function create() {
		...
		$userInfo=$this->Staff_Model->getRoleByStaffId($this->session->usr_id);
		if($userInfo['role_name']=='Estimator' || $this->session->admin=='1'){
			$data['customers']=$this->Customers_Model->get_all_customers();
		}else{
			$data['customers']=$this->Customers_Model->get_all_customers_by_acccess();
		}
		...
		if ( $this->Privileges_Model->check_privilege( 'estimations', 'create' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				foreach(array_filter($items[$y]['item_name']) as $k => $item_name){
					...
					$item_total_price=str_replace( ',', '', $items[$y]['amount'][$k] );
					
					$item_data['item_name'] = $item_name;
					$item_data['item_description'] = $items[$y]['item_description'][$k];
					$item_data['item_discount'] = $items[$y]['item_discount'][$k];
						$item_data['item_total_price'] = $item_total_price;
					...
					$item_data['sub_tot_cost'] = $items[$y]['sub_tot_cost'][$k];
					$item_data['sub_tot_sp'] = $items[$y]['sub_tot_sp'][$k];
					...
					'item_total_price'=>$item_data['item_total_price'],
				...
				$staffname = $this->session->staffname;
					...
					$this->db->insert( 'estiimation_notifications', array(
						...
						'detail' => ( '' . $staffname .' req '.$estimation_id),
						'staff_id' => $loggedinuserid,
						'perres' => $staffavatar,
						'customer_id' => $client_id,
						'number' => ('EST- '.$estimation_id),
						'first_levelinfo' => $company,
						'value' => $estimation_total_amt,
						'customer_name' => $project_name,
						'request_type'=> "estimation",
						'target' => '' . base_url( 'estimations/view/' . $estimation_id . '' ) . ''
					) );
...
function get_client_contacts(){
	...
	$supplier= ' <select class="form-control salesteam my-select" data-live-search="true"  name="salesteam" id="salesteam" ><option value="">Select Sales Person </option>';
...
$alldata='';
		...
		if($proposals) {
			if ( $this->Privileges_Model->check_privilege( 'estimations', 'edit' ) ) {
				...
				if ( isset( $pro[ 'estimation_id' ] ) ) {
					if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				$setStatus=($proposals['estimate_status']=='Quote Request' ? 'Draft' :$proposals['estimate_status']);
				$estimate_by=($proposals['estimate_by']=='0' ? $this->session->userdata( 'usr_id' ) : $proposals['estimate_by']);
				$estimate_on=($proposals['estimate_by']=='0' ? date('Y-m-d H:i:s') : $proposals['estimate_on']);
				$project_name = $this->input->post('estimation_project_name');
				$special_notes = $this->input->post('special_notes');
				$estimatestatus = $setStatus;
				$client_id = $this->input->post('client_id');
				$client_contact_id = $this->input->post('client_contact_id');
				$estimation_total_cost_amt = $this->input->post('estimation_total_cost_amt');
				$subtotal_amt =  $this->input->post('subtotal_amt');
				...
				redirect(base_url('estimations/create/'. $id));
                }  
				...
				if(sizeof($order) > 0) {
					$this->db->delete( 'items', array( 'relation' => $order['id'], 'relation_type'=>'order' ));
					$this->db->delete( 'orders_sub_items', array( 'order_id' => $order['id']));
					
					$amountData=array(
						'sub_total' => $subtotal_amt + $discount,
						'total_discount' => $discount,
						'total_tax' => $estimation_tax_amount,
						'total' => $estimation_total_amt,
						'cost_amount' => $estimation_total_cost_amt,
						'profit_amt' => $estimation_profit_amt
					);
					$this->db->where('id', $order['id'])->update('orders',$amountData);
				}
				...
				foreach(array_filter($items[$y]['item_name']) as $k => $item_name){
					$unitprice=str_replace( ',', '', $items[$y]['unit_price'][$k] );
					$amount=str_replace( ',', '', $items[$y]['amount'][$k] );
					$item_no_discount1=str_replace( ',', '', $items[$y]['item_no_discount'][$k] );
					
					
					$item_data['item_name'] = $item_name;
					$item_data['item_description'] = $items[$y]['item_description'][$k];
					$item_data['item_discount'] = $items[$y]['item_discount'][$k];
						$item_data['item_total_price'] = $item_no_discount1;
					$item_data['estimation_id'] = $estimation_id;
 					$item_data['quantity'] = $items[$y]['qty'][$k];
					$item_data['unit_price'] = $unitprice;
					$item_data['tax'] = $items[$y]['tax'][$k];
					...
					//Rima Quotes Module
					$quoteSubItemId = $this->Estimations_Model->insert_quote_sub_items($sub_params,$quoteItemId);
	...
	function view( $id ) {
		...
		$data['client_contact_person'] = $this->Estimations_Model->get_customer_contact($est['customer_contact_id']);
		$data['estimation_main_items'] = $this->Estimations_Model->get_estimation_main_items($id);
		$data['estimation_sub_items'] = $this->Estimations_Model->get_estimation_sub_items($id);
		$data['estimation_documents'] = $this->Estimations_Model->get_estimation_documents($id);
		...
		$data['orderResult']=$this->Orders_Model->getRequestByEstID($id );
		$data['special_notes'] = $est['special_notes'];
		$data['description'] = $est['description'];
		$data['declined_notes'] = $this->db->get_where( 'rejected_notes', array( 'relation' => $id, 'relation_type' => 'estimation'))->row_array();
		$data['estimateByStaff'] = ($est[ 'estimate_by' ] !='0' ? $this->Orders_Model->get_staff_name_by_id($est[ 'estimate_by' ]) : '');
		$data['estimateDate'] =($est[ 'estimate_by' ] !='0' ? date('d-m-Y g:i a', strtotime( $est['estimate_on'])) : '');
		$data['revisions'] = $this->Privileges_Model->get_revision_by_id($id,'estimations');
	...
	function markas() { 
		if ( $this->Privileges_Model->check_privilege( 'estimations', 'edit' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				$this->db->insert( 'estiimation_notifications', array(
					'date' => date( 'Y-m-d H:i:s' ),
					'detail' => ( '' . $staffname .' sent '.'EST-'. $_POST[ 'proposal_id' ].' for approval '),
					'staff_id' => $loggedinuserid,
					'perres' => $staffavatar,
					'target' => '' . base_url( 'estimations/view/' . $_POST[ 'proposal_id' ] . '' ) . ''
				) );
	...
	function add_declined_msg(){
		$statusname = 'Declined';
		$validation_msg =lang('Please Enter Reason For Declined');
		
		if ( isset( $_POST ) && count( $_POST ) > 0 && $_POST[ 'estdeclined_msg' ] !='') {
			$estimationId = $_POST[ 'estimation_id' ];
			$msg=$_POST[ 'estdeclined_msg' ];
			$rejectedparams = array(
				'relation_type' => 'estimation',
				'relation' => $estimationId,
				'status' => $statusname,
				'notes' => $_POST[ 'estdeclined_msg'],
				'created' => date('Y-m-d h:i:s')
			);
			$this->db->insert( 'rejected_notes', $rejectedparams );
			$this->db->where( 'estimation_id', $estimationId)->update( 'estimations', array( 'estimate_status' => $statusname ) );
			
			$order = $this->db->get_where('orders', array( 'estimation_id' => $estimationId))->row_array();
			if(sizeof($order) > 0) {
				$rejectedOrdparams = array(
				...
				$this->db->insert( 'rejected_notes', $rejectedOrdparams );
				$this->db->where('id', $order['id'])->update( 'orders', array('status_id'=>'5'));
			}
			
			$data['success'] = true;
			$data['message'] = lang( 'estimation' ).' '.lang('markas').' '.$statusname ;
		}else{
			$data['success'] = false; 
	...
	function exist_order_addnote() {
		$order = $data = array();
		$order = $this->db->get_where( 'orders', array( 'estimation_id' => $_POST[ 'relation']))->row_array();
		if(sizeof($order) > 0) {
			$params = array(
