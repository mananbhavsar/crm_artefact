		...
		if ( $this->Privileges_Model->check_privilege( 'staff', 'all' ) ) {
			...
			while(date($strt)<=date($end))
			{
				...
				foreach($attend_details as $details) {
					...
					$timings = $this->Attendance_Model->get_attendance_time(($details["empl_id"]));
					$st_time = strtotime($timings['start_time']);
					$et_time = strtotime($timings['end_time']);
					$next_day_continuation = $this->Attendance_Model->next_day_continuation($details["punch_date"],$details["empl_id"]);
					if($time1 != '' && $time2 != ''){
						if($time1 < $st_time ){
							$pr_status = 'Present';
						}
			...
			$data['totalpresent']=$totalpre;
			$data['totalabsent']=$totalabsent;
			$data['attendace_result']=$data1;
		  ...
		  $sickleaves=0;
					...
					$data['attendace_result1'] = $get_staff_attendance=$this->Staff_Model->get_staff_attendance($first_date,$last_date,$staff[ 'id' ]);
...
*/
		...
		if ( $this->Privileges_Model->check_privilege( 'staff', 'all' ) ) {
			...
			foreach ( $staffs as $staff ) {
				...
				$data_staffs[] = array(
					...
					'color' => $color,
					'birthday' => $bdate,
					'status_type'	=> $staff[ 'statuscolor' ],
					'status'		=>  $staff[ 'statusname' ],
		...
		$hasError = false;
				...
				$params = array(
						...
						'updatedAt'=>date('Y-m-d H:i:s')
								...
								redirect(base_url('staff/staffmember/'.$id.'#tab2'));
	}
	
	function update_appraisal_details($id,$appraisal_id=0)
	{
		
		if($this->input->server("REQUEST_METHOD")=="POST"){ 
			$data = $this->input->post();
			$data['staff_id'] = $id;
			$appraisal_id= isset($_POST['id'])?$_POST['id']:0;
			$staff = $this->Staff_Model->get_staff( $id );
			if($appraisal_id!=0){
				$amt=0;
				$sql = $this->db->query("SELECT * FROM staff_increment WHERE id = '$appraisal_id'");
				$row = $sql->row();
				if($row->increment_type == 'Fixed Portion'){
					if($row->type_of_amount=='plus'){
						$amt=$staff['basic_salary'] - $row->increment_amount;
					}
					else if($row->type_of_amount=='minus'){
						$amt=$staff['basic_salary'] + $row->increment_amount;
					}
		...
		/*	$old_doc = $this->input->post('old_appraisal_data');
			...
			$totalamt=0;
			$staff1 = $this->Staff_Model->get_staff( $id );
			if($data['increment_type'] == 'Fixed Portion'){
			    if($data['type_of_amount']=='plus'){
					$totalamt=$data['increment_amount']+$staff1['basic_salary'];
				}else if($data['type_of_amount']=='minus'){
					if($data['increment_amount']>$staff1['basic_salary']){
						$error='Amount should not be greater than basic Salary';
					}else{
						$totalamt=$staff1['basic_salary'] - $data['increment_amount']  ;
					}
				}
				if(isset($error)){
					$this->session->set_flashdata( 'apprmessage', $error );
					redirect('staff/staffmember/'.$id.'#tab3','refresh');
				}
			    ...
			    $params = array(
				 ...
				 if($data['type_of_amount']=='plus'){
					$totalamt=$data['increment_amount']+$staff1['allowance'];
				}else if($data['type_of_amount']=='minus'){
					if($data['increment_amount']>$staff1['allowance']){
						$error='Amount should not be greater than basic Salary';
					}else{
						$totalamt=$staff1['allowance'] - $data['increment_amount']  ;
					}
				}
				if(isset($error)){
					$this->session->set_flashdata( 'apprmessage', $error );
			    ...
			    $params = array(
				 ...
				 if($data['type_of_amount']=='plus'){
					$totalamt=$data['increment_amount']+$staff1['vehicle_amound'];
				}else if($data['type_of_amount']=='minus'){
					if($data['increment_amount']>$staff1['vehicle_amound']){
						$error='Amount should not be greater than basic Salary';
					}else{
						$totalamt=$staff1['vehicle_amound'] - $data['increment_amount']  ;
					}
				}
				if(isset($error)){
					$this->session->set_flashdata( 'apprmessage', $error );
				...
				if($data['type_of_amount']=='plus'){
					$totalamt=$data['increment_amount']+$staff1['accom_amound'];
				}
				else if($data['type_of_amount']=='minus'){
					if($data['increment_amount']>$staff1['accom_amound']){
						$error='Amount should not be greater than basic Salary';
					}else{
						$totalamt=$staff1['accom_amound'] - $data['increment_amount']  ;
					}
				}
				if(isset($error)){
					$this->session->set_flashdata( 'apprmessage', $error );
			...
			if($appraisal_id!=0){
				$this->Staff_Model->updateappraisal_increment($appraisal_id,$data);
				$this->Staff_Model->employee_history('Updated','Appraisal Info');
			}
			else{
	...
	function GetAppraisal($appraisal_id){
		$data = $this->Staff_Model->GetAppraisal($appraisal_id);
		echo(json_encode($data));
	}
	function GetWarning($warning_id){
		$data = $this->Staff_Model->GetWarning($warning_id);
		echo(json_encode($data));
	}
	function GetTools($tools_id){
		$data = $this->Staff_Model->GetTools($tools_id);
		echo(json_encode($data));
	}
	function add_warning($id)
	{
	...
	{
		if($this->input->server("REQUEST_METHOD")=="POST"){ 
			$data['warning_date'] = $this->input->post('warning_date');
			$data['date_of_incident'] =  $this->input->post('date_of_incident');
			$data['warning_type'] = $this->input->post('warning_type');
			$data['type_of_violation'] = $this->input->post('type_of_violation');
			$data['action'] = $this->input->post('action');
			$data['employee_signature'] = $this->input->post('employee_signature');
			$data['user_id'] = $this->session->userdata( 'usr_id' );
			$warning_id = $this->input->post('warning_id');
			$this->Staff_Model->employee_history('Updated','Warnings');
			$this->Staff_Model->update_warning($data,$warning_id);
	...
	function update_tools($id)
	{
		if($this->input->server("REQUEST_METHOD")=="POST"){ 
			...
			$data['item_discription'] =  $this->input->post('item_discription');
			...
			$this->Staff_Model->employee_history('Updated','Tools & Assets');
			$this->Staff_Model->update_tools($data,$tools_id);
	...
	function update_grade(){
     ...
     function delete_notes($id,$staff_id)
     {
      ...
      //  $this->session->set_flashdata('message', 'Successfully Deleted');
        	...
        	redirect('staff/staffmember/'.$staff_id.'#tab7','refresh');
     }
     function delete(){
         $id = $this->input->post('id');
	...
	function update_documents($id)
	{
		...
		redirect('staff/staffmember/'.$id.'#tab1','refresh');
		}
	...
	function payroll(){
		...
		$statusdata = $this->db->select( 'id,name' )->from( 'status')->get()->result();
        ...
        foreach ( $statusdata as $status ) {
			$string = str_replace(' ', '', $status->name);
			$data[$string]=$this->Staff_Model->get_status($status->id);
		};
		$data['total'] = $this->Staff_Model->get_status('all');
			$data['success'] = true;
	...
	function download() {
		 $docfile = $this->input->get('val');
        if (isset($docfile)) {
            if (is_file('uploads/staff_documents/'.$docfile)) {
                $this->load->helper('file');
                $this->load->helper('download');
                $data = file_get_contents('uploads/staff_documents/' .$docfile);
  ...
  <div class="form-row">
    ...
    <div class="input-group">
       ...
       <input type="text" name="leave_start_date" class="form-control " id="leave_start_date1" required="" autocomplete="off" readonly value="'.$leavedet['leave_start_date'].'"><span class="input-group-addon"><i class="glyphicon glyphicon-th"></i></span>
        </div>
	...
	<div class="form-group col-md-6" >
     ...
     <div class="input-group date">
        <input type="text" name="rejoin_date" class="form-control " id="rejoin_date1" onchange="select_rejoin1();" required=""  value="'.$leavedet['rejoin_date'].'" readonly ><span class="input-group-addon" ><i class="glyphicon glyphicon-th"></i></span>
        </div>
 ...
 <div class="form-group col-md-6">
	...
	////Start Status Settings Code by Namrta //////////
	function get_status() {
		$statusarr = $this->Staff_Model->get_all_Status();
		...
		if ( $this->Privileges_Model->check_privilege( 'staff', 'create' ) ) {
			if ( isset( $_POST ) && count( $_POST ) > 0 ) {
				...
				if (!$hasError) {
					...
					if ($id) {
						$data['success'] = true;
						$data['message'] = lang('status'). ' ' .lang('createmessage');
						echo json_encode($data);
					} else {
						$data['success'] = false;
						$data['message'] = lang('errormessage');
	...
	function update_status( $id ) {
		if ( $this->Privileges_Model->check_privilege( 'staff', 'edit' ) ) {
			$data[ 'status' ] = $this->Staff_Model->get_set_status( $id );
			if ( isset( $data[ 'status' ][ 'id' ] ) ) {
				if ( isset( $_POST ) && count( $_POST ) > 0 ) {
					...
					if (!$hasError) {
						...
						$result=$this->Staff_Model->update_status( $id, $params );
						if ($result) {
							$data['success'] = true;
							$data['message'] = lang('status'). ' ' .lang('updatemessage');
							echo json_encode($data);
						} else {
							$data['success'] = false;
							$data['message'] = lang('errormessage');
	...
	function remove_status( $id ) {
		if ( $this->Privileges_Model->check_privilege( 'staff', 'delete' ) ) {
			$status = $this->Staff_Model->get_set_status( $id );
			if ( isset( $status[ 'id' ] ) ) { 
				if ($this->Staff_Model->get_status($id) == 0) {
					$this->Staff_Model->remove_status( $id );
					$data['success'] = true;
					$data['message'] = lang('status'). ' ' .lang('deletemessage');
					echo json_encode($data);
				} else {
					$data['success'] = false;
					$data['message'] = $data['message'] = lang('status').' '.lang('is_linked').' '.lang('with').' '.lang('staff').', '.lang('so').' '.lang('cannot_delete').' '.lang('status');
					echo json_encode($data);
				}
			}
		} else {
	...
	////End Status Settings Code by Namrta //////////
	
	
	function update_heading(){
	    
		
		$heading = $this->input->post( 'docheading' );
				...
				$staffid = $this->input->post( 'staffid' );
					...
					$params = array(
								...
								$response = $this->Staff_Model->update_heading( $docid, $params );
									redirect(base_url('staff/staffmember/'.$staffid));
		//$this->load->view('doc_heading',$data);
	}
}