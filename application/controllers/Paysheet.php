...
class Paysheet extends CIUIS_Controller {
	...
	/*	 $day = date('d'); 
	  ...
	  // echo $day.'--'.$month.'--'.$year; die;
	   if($day > '25'){
	       $data['from_date'] = $year.'-0'.$previous.'-25';
	       $data['to_date'] = $year.'-'.$month.'-25';
	...
	$data['staff'] = $staff;
		...
		foreach($staff as $val){
			$emp = $this->input->post('empl_id'.$val['id']);
if($emp != ''){			
			$from_date = $this->input->post('from_date');
			$to_date = $this->input->post('to_date');
			$staff_id = $val['id'];
			$basic_salary = $this->input->post('basic_salary'.$val['id']);
			$allowance = $this->input->post('allowance'.$val['id']);
			$ot_hours = $this->input->post('ot_hours'.$val['id']);
			$ot_per_hour = $this->input->post('ot_per_hour'.$val['id']);
			$ot_amount = $this->input->post('ot_amount'.$val['id']);
			$advance = $this->input->post('advance'.$val['id']);
			$incentives = $this->input->post('incentives'.$val['id']);
			$total = $this->input->post('tot_sal'.$val['id']);
			...
			$this->db->insert('payslip',$params);
				...
				echo '<script>alert("Payroll Generated Successfully!");</script>';
