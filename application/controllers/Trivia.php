	...
	function addnote() {
		if ( isset( $_POST ) && count( $_POST ) > 0 ) {
			...
			if (!$hasError) {
				...
				$data[ 'insert_id' ] = $this->db->insert_id();
				if($_POST['relation_type']=='order'){
					$historyparams=array('module'=>'orders','relation_id'=>$_POST[ 'relation' ],'type'=>'Added Note');
					$this->Privileges_Model->create_history($historyparams);
				}
				if($_POST[ 'relation_type' ] == 'lead') {
					$this->db->where('id', $_POST['relation']);
					$this->db->update('leads',array('dateassigned'=>date('Y-m-d H:i:s')));
				}
	...
	function set_onsite_visit() {
		if ( isset( $_POST ) && count( $_POST ) > 0 ) {
			...
			if ($title == '') {
				...
				$hasError = true;
				$data['message'] = lang('selectinvalidmessage'). ' ' .lang('start'). ' ' .lang('date');
			} else if ($end == '') {
				$hasError = true;
				$data['message'] = lang('selectinvalidmessage'). ' ' .lang('end'). ' ' .lang('date');
			} else if (strtotime($end) < strtotime($start)) {
				$hasError = true;
				$data['message'] = lang('startdate').' '.lang('date_error'). ' ' .lang('end'). ' ' .lang('date');
			} else if ($description == '') {
				$hasError = true;
			...
			if (!$hasError) {
				...
				$data['success'] = true;
				$data['message'] = lang('onsite_visit_added');
				echo json_encode($data);
			}
		}
	}
