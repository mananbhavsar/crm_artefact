...
function get_countrycode($id) {
	$states_data = array();
	$states = file_get_contents( 'assets/json/countries.json' );
	$obj = json_decode($states,true); 
	foreach($obj as $state) {
		if($state['id'] == $id) {
			$states_data = $state['dialingcode'];
			break;
		}
	}
	return $states_data;
...
function get_number_estimate($table, $id, $name, $configname = NULL){
	$ci =& get_instance();
	$response = $ci->db->get_where($table,array('estimation_id'=>$id))->row_array();
	if($response[$name.'_number']){
		return $response[$name.'_number'];
	} else{
		$appconfig = get_appconfig();
		$number = $appconfig[$configname.'_prefix'].str_pad( $response['estimation_id'], 6, '0', STR_PAD_LEFT );
		return $number;
	}
}
...
function check_privilege ( $type, $privileges ) { 
	...
	$staffpermissions = $ci->db->get_where('individual_role_permissions', array('permission_id' => $permission_id, 'role_id' => $role_id,'staff_id'=>$staff_id))->row_array();
	if($staffpermissions){
...
function check_approval($permissiontype) { 
	$ci =& get_instance();
	$approvalResult=$ci->Privileges_Model->has_approval($permissiontype); 
    return 	$approvalResult;
}


function check_approval_data($requestsData,$maxvalue,$comperKey){
	if($comperKey=='NotAccess'){
	  foreach($requestsData as $key=>$value){
		  $requestsData[$key]['showAccess']='0';
	  }
	}else if($comperKey=='Level'){
	   foreach($requestsData as $key=>$value){
		  if($value['approve_level'] == $maxvalue){  
			$requestsData[$key]['showAccess']='1';
		  }else{
			  $requestsData[$key]['showAccess']='0';
		  }
	  }
...
function check_approval_data_ForId($requestsData,$maxvalue,$comperKey){
	if($comperKey=='NotAccess'){
	  foreach($requestsData as $key=>$value){
		  $requestsData['showAccess']='0';
	  }
	}else if($comperKey=='Level'){
	   foreach($requestsData as $key=>$value){
		  if($requestsData['approve_level'] == $maxvalue){ 	  
			$requestsData['showAccess']='1';
		  }else{
			  $requestsData['showAccess']='0';
		  }
	  }
		...
		foreach($requestsData as $key=>$value){
		  if($requestsData[$comperKey] <= $maxvalue){  
			$requestsData['showAccess']='1';
		  }else{
			  $requestsData['showAccess']='0';
		  }
