	...
	function getNoteBookList($searchdata='') {
		...
		if(sizeof($notebooksData) > 0){
			foreach($notebooksData as $k => $result) {
				...
				$page_name = $this->CI->Notebooks_Model->get_page_names($result['notebook_id']);
				...
				$finaldata .='<td class="md-cell"><strong>'.$page_name['page_names'].'</strong></td>';
				...
				$finaldata .='<td class="md-cell">';
				$finaldata .=' <div style="margin-top: 5px;" data-toggle="tooltip" data-placement="left" title="" class="assigned-staff-for-this-lead user-avatar"><img src="'.base_url().'uploads/images/'.$result['staffavatar'].'"></div> ';            
