...
function Ciuis_Controller($scope, $http, $mdSidenav, $filter, $interval, $mdDialog, $element, $mdToast, $log) {
	...
	$scope.get_contact_permissions = function () {
		$http.get(BASE_URL + 'api/contact_permision').then(function (Permissions) {
	...
	$scope.close = function () {
		...
		$mdSidenav('EstNotifications').close();
	...
	$scope.markAllEstAsRead = function() {
		var dataObj = $.param({});
		var posturl = BASE_URL + 'api/mark_estread_ntf';
		$http.post(posturl, dataObj, config)
		.then(function (response) {
			$http.get(BASE_URL + 'api/estnotifications').then(function (Notifications) {
				$scope.estnotifications = estnotifications.data;
			});
			//$scope.stats.tbs = '0';
		}, function (error) {}
			...
			.then(
				function (response) {
					...
					if (response.data.success) {
						...
						globals.mdToast('success', $scope.lang.todoadded);
						$scope.tododetail = '';
					}
				},
				function (response) {
		...
		$http.post(BASE_URL + 'trivia/donetodo', dataObj, config)
			.then(
				function (response) {
					...
					globals.mdToast('success', $scope.lang.tododone);
				},
				function (response) {
					console.log(response);
				}
	...
	$scope.InvoiceCancel = function (e) {
		...
		$http.post(posturl, dataObj, config)
			.then(
				function (response) {
					globals.mdToast('success', INVMARKCACELLED);
					$('.toggle-due').hide();
					$('.toggle-cash').hide();
	...
	$scope.EstNotificationRead = function (index) {
		var estnotifications = $scope.estnotifications[index];
		var posturl = BASE_URL + 'trivia/mark_read_estnotification/' + estnotifications.id;
		$http.post(posturl, config)
			.then(
				function (response) {
					console.log(response);
					window.location.href = estnotifications.target;
				},
				function (response) {
					console.log(response);
				}
	...
	$scope.GetEstNotificationcount = function (index) {
		var estnotifications = $scope.estnotifications[index];
		var posturl = BASE_URL + 'trivia/get_read_estnotification/' + estnotifications.id;
		$http.post(posturl, config)
			.then(
				function (response) {
					console.log(response);
	...
	$scope.ChangeTicketStatus = function () {
		...
		$http.post(BASE_URL + 'tickets/chancestatus', dataObj, config)
			.then(
				function (response) {
					globals.mdToast('success', TICKSTATUSCHANGE);
...
function Sidebar_Controller($scope, $http, $mdSidenav, $filter, $interval, $mdDialog, $element, $mdToast, $log) {
	...
	$scope.loadMoreLogs = function() {
		$scope.getLogs = true;
		$http.get(BASE_URL + 'api/logs/loadMore').then(function (Logs) {
			$scope.logs = Logs.data;
			$scope.getLogs = false;
		});
	}
